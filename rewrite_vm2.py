with open('src/regex_vm.npk', 'w') as f:
    f.write("""// regex_vm.npk — Thompson VM Core Engine Execution
...
func:apush_list = int64(int64:list_ah, int64:val, int64:start_idx, int64:captures_ptr) {
    ...
    if (captures_ptr == 0i64) {
        ...
        drop npk_mem_copy(element_ptr + 16i64, captures_ptr, 80i64);
    }
...
func:add_state = int64(int64:start_state, int64:list_ah, int64:visit_gen, int64:visited_arr, int64:work_ah, int64:idx, int64:text_len, int64:start_idx, int64:captures_ptr, int64:arena_data, int64:temp_caps_scratch) {
    ...
    when (raw asize_list(work_ah) > 0i64) {
        ...
        int64:state_captures_raw = raw apop_captures(work_ah);
        
        drop npk_mem_copy(temp_caps_scratch, state_captures_raw, 80i64);
        int64:state_captures = temp_caps_scratch;
...
pub func:regex_vm_match = MatchResult(int64:arena_ptr, int64:start_state, int64:text, int64:text_len, int64:initial_idx) {
    ...
    int64:arena_data = npk_mem_read_int64(arena_ptr, 16i64);
    
    int64:batch_mem = npk_core_alloc(capacity * 96i64 * 3i64 + 24i64 + capacity * 8i64 + 160i64);
    ...
    int64:temp_caps_scratch = batch_mem + capacity * 288i64 + 24i64 + capacity * 8i64;
    int64:final_caps_buf = temp_caps_scratch + 80i64;
    ...
    int64:final_end_idx = -1i64;
    
    when (running == 1i32) {
        if (matched == 0i32) {
            drop add_state(start_state, clist_ah, visit_gen, visited_arr, work_ah, idx, text_len, idx, 0i64, arena_data, temp_caps_scratch);
        }
        ...
        if (raw asize_list(clist_ah) == 0i64) {
            ...
            when (ti < sz) {
                ...
                if (opcode == OP_MATCH) {
                    if (matched == 0i32 || state_start_idx < final_start_idx || (state_start_idx == final_start_idx && idx > final_end_idx)) {
                        matched = 1i32;
                        final_start_idx = state_start_idx;
                        final_end_idx = idx;
                        drop npk_mem_copy(final_caps_buf, state_captures, 80i64);
                    }
                } else if (c != -1i64) {
                    if (opcode == OP_CHAR) {
                        int32:match_char = s_ptr->match_char;
                        if (c == (match_char => int64)) {
                            int64:out1 = s_ptr->out1;
                            drop add_state(out1, nlist_ah, visit_gen, visited_arr, work_ah, next_idx, text_len, state_start_idx, state_captures, arena_data, temp_caps_scratch);
                        }
                    } else if (opcode == OP_ANY) {
                        int64:out1 = s_ptr->out1;
                        drop add_state(out1, nlist_ah, visit_gen, visited_arr, work_ah, next_idx, text_len, state_start_idx, state_captures, arena_data, temp_caps_scratch);
                    } else if (opcode == OP_CHAR_CLASS) {
                        int32:is_inverted = s_ptr->match_char; 
                        if (c <= 255i64) {
    ...
    if (matched == 1i32) {
        mr.matched = 1i32;
        mr.start_idx = @cast_unchecked<int32>(final_start_idx);
        mr.end_idx = @cast_unchecked<int32>(final_end_idx);
        int64:ci = 0i64;
        when (ci < 20i64) {
            mr.captures[ci] = npk_mem_read_int32(final_caps_buf, ci * 4i64);
            ci = ci + 1i64;
        }
    } else {
        mr.matched = 0i32;
